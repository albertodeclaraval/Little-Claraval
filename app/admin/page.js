...
var colors = { pergamino: '#F8F1E6', vino: '#782F40', oro: '#B4903E', texto: '#2C1810' }
...
export default function AdminPage() {
  ...
  useEffect(function() {
    supabase.auth.getSession().then(function(res) {
      if (res.data.session) {
        ...
        if (ADMIN_EMAILS.includes(res.data.session.user.email)) {
          setAuthorized(true)
          loadCodes()
        }
      }
  ...
  function loadCodes() {
    supabase.from('promo_codes').select('*').order('created_at', { ascending: false }).then(function(res) {
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '8px', padding: '1.5rem', marginBottom: '1.5rem', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ marginBottom: '0.75rem' }}>
            ...
            <input type="number" value={days} onChange={function(e) { setDays(e.target.value) }} style={{ width: '100%', padding: '0.5rem', borderRadius: '6px', border: '1px solid #ddd', color: colors.texto }} />
          </div>
          <div style={{ marginBottom: '0.75rem' }}>
            <label style={{ display: 'block', fontSize: '0.8rem', marginBottom: '0.25rem', fontWeight: 'bold' }}>Usos maximos</label>
            <input type="number" value={maxUses} onChange={function(e) { setMaxUses(e.target.value) }} style={{ width: '100%', padding: '0.5rem', borderRadius: '6px', border: '1px solid #ddd', color: colors.texto }} />
          </div>
          <button onClick={handleCreate} style={{ backgroundColor: colors.vino, color: 'white', border: 'none', borderRadius: '6px', padding: '0.75rem 1.5rem', cursor: 'pointer', width: '100%', fontSize: '0.9rem' }}>
            Generar codigo
          </button>
            ...
            return (
              <div key={c.id} style={{ borderBottom: '1px solid #eee', padding: '0.75rem 0' }}>
                ...
                <div style={{ fontSize: '0.8rem', color: '#888', marginTop: '0.25rem' }}>
                  {c.duration_days} dias | {c.current_redemptions}/{c.max_redemptions} usos
                </div>
              </div>
            )
          })}
