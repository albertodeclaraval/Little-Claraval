...
async function findReflection(sb, lang, season, cycle, feastKey, week) {
...
// GET /api/admin/reflexiones?season=ordinary&cycle=II&feast_key=&week=8
export async function GET(request) {
  ...
  var [es, en] = await Promise.all([
    findReflection(sb, 'es', season, cycle, feastKey, week),
    findReflection(sb, 'en', season, cycle, feastKey, week),
  ])
...
// Body: { lang, season, cycle, feast_key, week, silence, meditative_phrase, inner_question, brief_prayer, spiritual_school, theme, tags }
export async function PATCH(request) {
  ...
  try {
    ...
    var sb = adminClient()
    var existing = await findReflection(sb, lang, season, cycle, feastKey, weekVal)
